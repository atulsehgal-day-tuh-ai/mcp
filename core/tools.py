...
class ToolManager:
    ...
    async def get_all_tools(cls, clients: dict[str, MCPClient]) -> list[Tool]:
        ...
        tools = []
        for client in clients.values():
            # Get the result object (the "box")
            result_wrapper = await client.list_tools()

            # Extract the actual list of tools from inside the box
            # The property is typically named 'tools'
            tool_list = result_wrapper.tools

            tools += [
                {
                    "name": t.name,
                    "description": t.description,
                    # Note: verify if it is 'inputSchema' or 'input_schema' 
                    # based on the Tool object definition, but we keep your existing convention:
                    "input_schema": t.inputSchema, 
                }
                for t in tool_list
            ]
    ...
    async def _find_client_with_tool(
        ...
        """Finds the first client that has the specified tool."""
        for client in clients:
            # Fix: Unwrap the result object to get the actual list
            result_wrapper = await client.list_tools()
            tools = result_wrapper.tools
            
            # Now we can safely iterate over 'tools'
            tool = next((t for t in tools if t.name == tool_name), None)
            if tool:
                return client
        return None
